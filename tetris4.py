...
def add_to_stop_all_block_list(stop_all_block_list, current_block, current_block_start_row, current_block_start_col):
    ...
    for row, line in enumerate(current_block):
        for col, block in enumerate(line):
            if block != '.':
                stop_all_block_list[min(current_block_start_row + row, BLOCK_ROW_NUM-1)][min(current_block_start_col + col, BLOCK_COL_NUM-1)] = "X"
...
def judge_move_right(current_block, current_block_start_row, current_block_start_col, stop_all_block_list):
    ...
    for col in range(len(current_block[0]) - 1, -1, -1):
        ...
        if 'X' in col_list:
            indices = [i for i, value in enumerate(col_list) if value == 'X']
            for j in indices:
                if stop_all_block_list[min(current_block_start_row+j, BLOCK_ROW_NUM-1)][min(current_block_start_col+col, BLOCK_COL_NUM-1)] == 'X':
                    return False
