...
def judge_move_right(current_block, current_block_start_row, current_block_start_col, stop_all_block_list):
    ...
    for col in range(len(current_block[0]) - 1, -1, -1):
        ...
        if 'X' in col_list:
            ...
            for j in indices:
                if stop_all_block_list[current_block_start_row+j][current_block_start_col+col] == 'X':
                    return False
...
def ai_suggest_best_position(current_block, stop_all_block_list, current_block_start_row_1, current_block_start_col_1):
    ...
    for rotation in range(4):
        ...
        for col in range(BLOCK_COL_NUM - len(current_block[0]) + 1):
            temp_board = simulate_drop(current_block, col, stop_all_block_list, current_block_start_row_1)

            cleared_lines = judge_lines(temp_board)
            holes = count_holes(temp_board)
            height = get_stack_height(temp_board)

            score = cleared_lines * 20 - holes * 40 - height * 10
...
def simulate_drop(block, start_col, stop_all_block_list, current_block_start_row_2):
    temp_board = [row[:] for row in stop_all_block_list]

    while judge_move_down(block, current_block_start_row_2 + 1, start_col,
                          temp_board):
        current_block_start_row_2 += 1

    add_to_stop_all_block_list(temp_board, block, current_block_start_row_2,
                               start_col)
